...
async function ensureDatabaseExists() {
  ...
  const database = process.env.DB_NAME || "track_daily";
...
async function applySchemaUpgrades() {
  await ensureColumn("users", "name", "name VARCHAR(255) NOT NULL DEFAULT ''");
  if (await columnExists("users", "username")) {
    await pool.query("ALTER TABLE users MODIFY username VARCHAR(255) NOT NULL DEFAULT ''");
    await pool.query("UPDATE users SET name = username WHERE (name IS NULL OR name = '') AND username IS NOT NULL");
  }
  ...
  // Nutrition profile and library updates
  await ensureColumn("users", "diet_preference", "diet_preference VARCHAR(255) NULL");
  await ensureColumn("users", "allergies", "allergies JSON NULL");

  await ensureColumn("exercise_library", "normalized_name", "normalized_name VARCHAR(255) NULL");
  await ensureColumn("exercise_library", "tracking_type", "tracking_type VARCHAR(50) NULL");
  await ensureColumn("exercise_library", "tags", "tags JSON NULL");
  await ensureColumn("exercise_library", "difficulty", "difficulty VARCHAR(50) NULL");
  await ensureColumn("exercise_library", "is_active", "is_active BOOLEAN NOT NULL DEFAULT TRUE");
  await ensureColumn("exercise_library", "source", "source VARCHAR(50) NOT NULL DEFAULT 'default'");
  await ensureColumn("exercise_library", "default_met", "default_met DECIMAL(5,2) NULL");
  await ensureColumn("exercise_library", "distance_multiplier", "distance_multiplier DECIMAL(5,2) NULL");
  await ensureColumn("exercise_library", "bodyweight_factor", "bodyweight_factor DECIMAL(5,2) NULL");
  await ensureColumn("exercise_library", "calorie_method", "calorie_method VARCHAR(50) NULL");
  await ensureColumn("exercise_library", "intensity_level", "intensity_level VARCHAR(50) NULL");
  await ensureColumn("exercise_library", "estimate_confidence", "estimate_confidence VARCHAR(50) NULL");
}
...
async function seedActivityLibraryToMySQL() {
  const fs = require('fs');
  const path = require('path');

  let seedPath = path.join(process.cwd(), 'database', 'seed', 'activity-library.json');
  if (!fs.existsSync(seedPath)) {
    seedPath = path.join(process.cwd(), '..', 'database', 'seed', 'activity-library.json');
  }
  if (!fs.existsSync(seedPath)) {
    console.error("activity-library.json not found during seeding");
    return;
  }
  
  try {
    const activities = JSON.parse(fs.readFileSync(seedPath, 'utf8'));
    for (const act of activities) {
      let sqlCategoryId = 'cat_strength';
      if (act.categoryId === 'cat-1') sqlCategoryId = 'cat_strength';
      else if (act.categoryId === 'cat-2') sqlCategoryId = 'cat_cardio';
      else if (act.categoryId === 'cat-3') {
        const nameLower = act.name.toLowerCase();
        if (nameLower.includes('yoga')) {
          sqlCategoryId = 'cat_yoga';
        } else {
          sqlCategoryId = 'cat_mobility';
        }
      }
      else if (act.categoryId === 'cat-4') sqlCategoryId = 'cat_sports';

      const id = act.id;
      const name = act.name;
      const normalized_name = act.normalizedName || name.toLowerCase().replace(/\s+/g, ' ');
      const tracking_type = act.trackingType || 'sets_reps_weight';
      const tags = JSON.stringify(act.tags || []);
      const difficulty = act.difficulty || 'beginner';
      const is_active = act.isActive !== false;
      const source = act.source || 'default';
      const created_by = act.createdByUserId || null;
      const default_met = act.defaultMet !== undefined ? act.defaultMet : null;
      const distance_multiplier = act.distanceMultiplier !== undefined ? act.distanceMultiplier : null;
      const bodyweight_factor = act.bodyweightFactor !== undefined ? act.bodyweightFactor : null;
      const calorie_method = act.calorieMethod || null;
      const intensity_level = act.intensityLevel || null;
      const estimate_confidence = act.estimateConfidence || null;

      const muscle_group = act.tags && act.tags[0] ? act.tags[0] : 'General';
      const equipment = act.tags && act.tags[1] ? act.tags[1] : 'Bodyweight';
      const exercise_type = tracking_type === 'sets_reps_weight' ? 'strength' :
                            tracking_type === 'duration_distance' ? 'cardio' :
                            tracking_type === 'duration_focus' ? 'mobility' : 'sports';
      ...
      await pool.execute(
        ...
        ON DUPLICATE KEY UPDATE
          ...
          default_met = VALUES(default_met),
          ...
          estimate_confidence = VALUES(estimate_confidence)`,
        [
          id, name, sqlCategoryId, muscle_group, equipment, exercise_type, default_duration, source === 'custom' || source === 'user', created_by,
          normalized_name, tracking_type, tags, difficulty, is_active ? 1 : 0, source,
          default_met, distance_multiplier, bodyweight_factor, calorie_method, intensity_level, estimate_confidence
        ]
      );
    }
  } catch (err) {
    console.error("Failed to seed activity library:", err);
  }
}
...
async function seedDemoUsers() {
  const demoUsers = [
    {
      ...
      name: "Demo Admin",
      role: "admin",
      passwordHash: "$2b$10$8jH4pE2cUqDiiz/k7gU5HedguYTPN3s26pDQox42dmkWeO9bfVq/G",
      age: null,
      gender: null,
      height: null,
      weight: null,
    ...
    },
    {
      ...
      name: "Demo User",
      role: "user",
      passwordHash: "$2b$10$sPUSdsccOUReRG2Yo3IfCOvriINfPB1D/kNVLs62j7WXfFf1kxLtS",
      age: 21,
      gender: "other",
      height: 170,
      weight: 70,
  ...
  for (const user of demoUsers) {
    if (hasUsername) {
      await pool.execute(
        `INSERT INTO users (
           id, username, email, name, role, password_hash, age, gender, height, weight,
...
async function seedDefaults() {
  ...
  await seedActivityLibraryToMySQL();
