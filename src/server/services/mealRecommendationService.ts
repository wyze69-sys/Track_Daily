...
import { FoodItem } from './nutritionImportService';
...
export const mealRecommendationService = {
  generateMealPlan(input: MealRecommendationInput): MealPlanRecommendation | { fallbackMessage: string } {
    ...
    if (input.dietPreference) {
      ...
      if (diet === 'vegan') {
        ...
        // Exclude food items where carbs comprise more than 20% of calories
        filteredFoods = filteredFoods.filter(food => {
    ...
    const recommendForSlot = (target: { cal: number, prot: number, carbs: number, fat: number }, keywords: string[]): RecommendedMealItem | null => {
      ...
      // Fallback if no matching foods by keyword
      ...
      for (const food of candidates) {
        // Ideal multiplier to match slot calories
        let multiplier = target.cal / food.calories;
        
        // Restrict multiplier to a realistic serving range [0.2, 5.0]
        ...
        // Calculate macro error (weighted by calorie counts to balance importance)
        const error = 
